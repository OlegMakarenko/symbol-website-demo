/*
 *
 * Copyright (c) 2019-present for NEM
 *
 * Licensed under the Apache License, Version 2.0 (the "License ");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 */

<template>
    <div class="link">
        <div class="inner-container">
            {{ caption }}
        
            <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.8877 4C7.04494 4 7.19302 4.07396 7.28746 4.19968L12.9218 11.6997C13.0555 
                    11.8776 13.0555 12.1224 12.9218 12.3003L7.28746 19.8003C7.19302 19.926 7.04494 
                    20 6.8877 20H4.63862C4.22638 20 3.99125 19.5292 4.23889 19.1996L9.42337 12.3004C9.55708 
                    12.1224 9.55708 11.8776 9.42337 11.6996L4.23889 4.80037C3.99125 4.47081 4.22638 
                    4 4.63862 4H6.8877ZM14.8877 4C15.045 4 15.193 4.07396 15.2875 4.19968L20.9218 
                    11.6997C21.0555 11.8776 21.0555 12.1224 20.9218 12.3003L15.2875 19.8003C15.193 
                    19.926 15.045 20 14.8877 20H12.6386C12.2264 20 11.9913 19.5292 12.2389 19.1996L17.4234 
                    12.3004C17.5571 12.1224 17.5571 11.8776 17.4234 11.6996L12.2389 4.80037C11.9913 4.47081 
                    12.2264 4 12.6386 4H14.8877Z"
                    class="arrow"
                />
            </svg>
        </div>
    </div>
</template>


<script>
export default {
    name: "Button",

    props: {
        caption: {
            type: String,
            default: " ",
        },
    },

    methods: {
        changeTheme(theme) {
            this.$store.dispatch("ui/changeTheme", theme);
        },
    },
};
</script>


<style lang="scss" scoped>
.link {
    display: inline-block;
    color: var(--color-button-text-unbounded);
    border-radius: 4px;
    text-align: center;
    text-decoration: var(--font-button-unbounded-text-decoration);
    font-size: var(--font-button-unbounded-font-size);
    font-family: var(--font-button-unbounded-font-family);
    font-weight: var(--font-button-unbounded-font-weight);
    font-style: var(--font-button-unbounded-font-style);
    font-stretch: var(--font-button-unbounded-font-stretch);
    letter-spacing: var(--font-button-unbounded-letter-spacing);
    line-height: var(--font-button-unbounded-line-height);
    text-transform: var(--font-button-unbounded-text-case);
    cursor: pointer;

    .inner-container {
        display: flex;
        align-items: center;

        .arrow {
            fill: var(--color-text-body);
        }
    }
}
</style>